...
import { HiCurrencyDollar, HiBolt, HiCalculator } from "react-icons/hi2";
...
function Products({ onNavigate }) {
  const products = [
    ...
    },
    {
      ...
      description:
        "Support school fees and education expenses with a longer repayment window.",
      detailedContent:
    ...
    },
    {
      ...
      title: "Welfare Loan",
      description:
        "A supportive loan option for member welfare and family needs.",
      detailedContent:
    ...
    },
    {
      ...
      description:
        "For bigger long-term projects with a higher limit and extended tenure.",
      detailedContent:
        ...
        <div className="max-w-6xl mx-auto px-4 pb-8">
          ...
          >
            <div>
              ...
              <p
                className={`mb-6 max-w-2xl text-sm sm:text-base ${isDark ? "text-gray-300" : "text-slate-600"}`}
              >
                Pick a product, set your amount, and adjust the repayment period
                to see an estimated monthly installment based on the listed
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <label className="block">
                  <span
                    className={`mb-2 block text-sm font-medium ${isDark ? "text-gray-200" : "text-slate-700"}`}
                  >
                    Loan product
                  </span>
                  <select
                    ...
                    onChange={(event) =>
                      setCalculatorProductId(event.target.value)
                    }
                ...
                <label className="block">
                  ...
                  />
                  <span
                    className={`mt-2 block text-xs ${isDark ? "text-gray-400" : "text-slate-500"}`}
                  >
                    Maximum for this product:{" "}
                    {formatCurrency(selectedCalculatorProduct.maxAmount)}
                  </span>
                </label>
              </div>
              ...
              <div className="mt-6">
                ...
                />
                <div
                  className={`mt-2 flex justify-between text-xs ${isDark ? "text-gray-400" : "text-slate-500"}`}
                >
                  <span>1 month</span>
                  <span>
                    {selectedCalculatorProduct.maxRepaymentMonths} months
                  </span>
                </div>
              </div>
            </div>
            ...
            >
              <div
                className={`mb-4 text-sm font-medium ${isDark ? "text-gray-300" : "text-slate-600"}`}
              >
                Estimated repayment summary
              </div>
              <div
                className={`mb-5 text-3xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}
              >
                {formatCurrency(loanSummary.monthlyPayment)}
                <span
                  className={`ml-2 text-base font-medium ${isDark ? "text-gray-400" : "text-slate-500"}`}
                >
                  / month
                </span>
              </div>
              ...
              <div className="space-y-3">
                <div
                  className={`flex items-center justify-between rounded-2xl px-4 py-3 ${isDark ? "bg-black/40" : "bg-slate-50"}`}
                >
                ...
                >
                  ...
                  <strong>
                    {(
                      selectedCalculatorProduct.monthlyInterestRate * 100
                    ).toFixed(1)}
                    %
                  </strong>
                </div>
              ...
              <p
                className={`mt-5 text-xs leading-6 ${isDark ? "text-gray-400" : "text-slate-500"}`}
              >
                This is an estimate for planning purposes and may vary from
                final approved loan terms, fees, or SACCO review outcomes.
              </p>
            </div>
          </div>
        </div>
                    ...
                    <ul className="space-y-1 mb-4 text-sm">
                      {product.features.slice(0, 3).map((feature, i) => (
                        <li key={i} className="flex items-center gap-2">
                          <span className="w-4 h-4 rounded-full bg-[#8cc63f]/20 flex items-center justify-center text-[#8cc63f] text-xs">
                            ✓
                          </span>
                          <span
                            className={
        ...
        <div className="py-0 mb-15">
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            >
              ...
              >
                Still searching? Let our support team point you in the right
                direction.
              </div>
              <button
                className="px-6 py-3 rounded-xl font-semibold bg-[#8cc63f] hover:bg-[#9fd858] text-white transition-all duration-300 hover:-translate-y-1 shadow-lg"
